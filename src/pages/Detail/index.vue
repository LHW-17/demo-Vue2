...
export default {
  ...
  methods: {
    ...
    async addShopCart() {
      //发请求通知服务器将产品加入数据库
      //使用了async的函数返回一个promise对象，所以可以根据promise对象判断成功或失败
      try {
        await this.$store.dispatch("addOrUpdateShopCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        //路由跳转 服务器存储成功，路由跳转传递参数,但由于参数过多不美观，采用传参简单数据加本地存储sessionStorage的方式传递
        //使用JSON转换成字符串存放到sessionStorage
