                ...
                <li :class="{ active: isActive }" @click="changeOrder(1)">
                  <a
                    >综合<span
                      ...
                      class="iconfont"
                      :class="{
                        'icon-arrowup': isArrowUp,
                        'icon-arrowdown': !isArrowUp,
                      }"
                    ></span
                  ></a>
                </li>
...
export default {
  ...
  methods: {
    //因为搜索功能可能多次使用，所以定义为函数
    getDate() {
      //对象合并Object.assign(a,b)将b合并到a (ES6新增)
      // Object.assign(this.searchParams, this.$route.query);
      this.$store.dispatch("getSearchList", this.searchParams);
    },
    //删除分类名称
    //若属性值为空，可以设为undefined，当前字段则不会带给服务器
    removeCategoryName() {
    ...
    removeKeyword() {
      ...
      if (this.$route.query) {
        this.$router.push({
          name: "search",
          query: { ...this.$route.query, keyword: undefined },
        });
      } else {
        this.$router.push({ name: "search" });
      }
  ...
  //当组件挂载完毕前执行一次，此时将home传递的参数整理，修改组件中searchParams响应式数据
  beforeMount() {
